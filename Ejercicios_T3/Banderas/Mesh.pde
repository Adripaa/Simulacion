...
class Malla
{
  ...
  int sizeX, sizeY; // Malla dimensions
  ...
  Malla (int t, int x, int y)
  {
    ...
    // Elasticity and damping are adjusted depending on the type of Malla
    switch (tipo)
    {
      case 1: 
        k = 400;
  ...
  {
    PVector Fw = new PVector(0,0,0);  // viento force
    ...
    // Projection of the normal on the viento
    projection = n.dot(viento);
    
    // The force of the viento is the projection of the normal on the viento
    Fw.set(abs(projection * viento.x), (projection * viento.y), (projection * viento.z));
